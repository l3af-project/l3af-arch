...
validate() {
    ...
    curl --noproxy '*' -sS http://${IP}:7080/l3af/configs/v1/lima0 >out.json 2>&1
    echo >> out.json
    if cmp -s out.json $1.json; then
        curl -sS --noproxy '*' http://${IP}:8899/bpfs/lima0 | jq ".[].ProgID" >progids.txt 2>err
        if [ -s err ]; then
            cat err
            logerr "curl request to debug api failed"
        fi
        curl -sS --noproxy '*' http://$IP:8899/bpfs/lima0 | jq ".[].Program.name" >names.txt 2>err
        if [ -s err ]; then
            cat err
            logerr "curl request to debug api failed"
        fi
...
rl_datapath_verification(){
    if grep -q "ratelimiting" names.txt;then
        before_rl_drop_count=`curl -sS --noproxy '*' $IP:8898/metrics | grep rl_drop_count_map_0_scalar | awk '{print $NF}'`
        before_rl_recv_count=`curl -sS --noproxy '*' $IP:8898/metrics | grep rl_recv_count_map_0_max-rate | awk '{print $NF}'`
        hey -n 200 -c 20 http://$IP:8080 > /dev/null
        for i in {1..120}; do
          after_rl_drop_count=`curl -sS --noproxy '*' $IP:8898/metrics | grep rl_drop_count_map_0_scalar | awk '{print $NF}'`
          after_rl_recv_count=`curl -sS --noproxy '*' $IP:8898/metrics | grep rl_recv_count_map_0_max-rate | awk '{print $NF}'`
...
cl_datapath_verification(){
    if grep -q "connection-limit" names.txt;then
        before_cl_recv_count=`curl -sS --noproxy '*' $IP:8898/metrics | grep cl_recv_count_map_0_scalar | awk '{print $NF}'`
        hey -n 200 -c 20 http://$IP:8080 > /dev/null
        for i in {1..120}; do
          after_cl_recv_count=`curl -sS --noproxy '*' $IP:8898/metrics | grep cl_recv_count_map_0_scalar | awk '{print $NF}'`
...
api_runner() {
    ...
    echo $num
    echo $IP
    curl -v -sS -X --noproxy '*' POST http://${IP}:7080/l3af/configs/v1/${name} -d "@${file}" > tmpr 2>&1
    if [ -s tmpr ]; then
        cat tmpr
