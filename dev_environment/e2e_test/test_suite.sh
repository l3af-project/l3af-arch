...
validate() {
    ...
    curl -sS http://${IP}:7080/l3af/configs/v1/ibpfbr >out.json 2>&1
    echo >> out.json
    if cmp -s out.json /root/l3af-arch/dev_environment/e2e_test/$1.json; then
...
api_runner() {
    name=$1
    file="/root/l3af-arch/dev_environment/e2e_test/$2"
    num=$3
    touch tmpr
...
api_runner "add" "traffic_mirroring_payload.json" 8
api_runner "delete" "del_traffic_mirroring.json" 9
l3afdID=$(pgrep l3afd)
kill -9 $l3afdID
